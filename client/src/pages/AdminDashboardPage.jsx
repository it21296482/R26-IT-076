        ...
        <section className="surface-panel fade-rise-delay-1">
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              ["03", "Outside conditions", "Input: dated stock-specific, local, and global coverage plus gold, crude oil, VIX, and USD/LKR histories.", "Finds relevant events, language tone, factor movement, stock association, and the practical business channel for the selected company.", "Novelty: this is not a generic news feed; event and sentiment context is connected to the selected stock, market-wide movement, measured factor association, and a company-specific business channel while separating association from proven cause. The live workflow is integrated; labelled CSE evaluation remains research work."],
              ["04", "Explainable risk impact", "Input: latest stock price, volume, recent and longer price averages, recent variability, gold, crude oil, and VIX.", "Classifies the combined financial-market risk as LOW, MEDIUM, or HIGH and identifies which stock or global indicators contributed most to that result.", "Novelty: combines stock risk classification with gold, oil, and global-market stress, then uses feature-level explanation to show why the category was produced instead of returning an unexplained label. The supplied model currently supports JKH, HHL, and LLUB; unsupported stocks are never mapped by proxy."],
          ...
          <div className="mt-6 rounded-[22px] border border-sky-100 bg-sky-50 p-5 text-sm leading-7 text-slate-700">
            <strong className="text-slate-950">Live demonstration:</strong> select BIL or JKH, upload its latest matching report, and click Analyze once. The server first refreshes every available stock quote from the official CSE trade summary, starts stages 01-03 together, runs stage 04 from the returned global indicators and latest stock history, then passes all available evidence to the plain-language integration layer. The combined result is stored for 24 hours. JKH demonstrates all four stages; BIL correctly reports that the supplied stage-04 model was not trained for that stock.
          </div>
        </section>
