...
const componentStatusStyles = {
  ...
  pending:
    "border-slate-300/20 bg-white/8 text-slate-200",
};
...
function InsightPreviewPage() {
  ...
  const [previewRequest] = useState(
    location.state || readStoredPreview()
  );
      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Unable to load the insight preview."
        );
      } finally {
        setLoading(false);
      }
  ...
  const sentimentRoute =
    hasPreview
      ? `/sentiment?symbol=${encodeURIComponent(
          selectedSymbol
        )}`
      : null;
    ...
    () => [
      {
        ...
        accent:
          "from-cyan-300/35 via-sky-300/10 to-transparent",
        ...
        evidence: selectedStock
          ? `${selectedStock.recordCount} market records checked.`
          : "Return to controls and select a company.",
        route: null,
      },

      {
        ...
        accent:
          "from-amber-300/35 via-orange-300/10 to-transparent",
        status: latestReport
          ? latestReport.processingStatus
          : "Optional",
        tone: latestReport
        ...
        input:
          ...
          selectedStock?.companyName ||
          "PDF report optional",
        evidence: latestReport
          ? "Report is queued for document parsing."
          : "Preview generated without a financial report.",
        route: null,
      },

      {
        ...
        accent:
          "from-fuchsia-300/30 via-rose-300/10 to-transparent",
        status: hasPreview
          ? "Prepared"
          : "Pending",
        tone: hasPreview
          ...
          : "pending",
        output: hasPreview
          ? "Market sentiment signal prepared"
          : "Not generated",
        input:
          selectedStock?.companyName ||
          selectedSymbol ||
          "Company selection required",
        evidence: hasPreview
          ? "Financial news and market-event context are ready for sentiment review."
          : "Generate preview to prepare this output.",
...

      {
        ...
        input:
          ...
          "Stock data required",
        evidence: hasPreview
          ? "Uncertainty is separated from the final insight."
          : "Risk output appears after setup.",
        route: null,
      },

      {
        ...
        accent:
          "from-blue-300/35 via-indigo-300/10 to-transparent",
        status: hasPreview
          ? "Prepared"
          : "Waiting",
        tone: hasPreview
        ...
        output: hasPreview
          ...
          : "Not generated",
        input:
          "Stock, report, sentiment, and risk outputs",
        evidence: hasPreview
          ? "Key factors are ready for explanation."
          : "XAI output appears after preview generation.",
        route: null,
      },
    ],
    [
          ...
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">
                Review stock data,
                financial report intake,
                sentiment, risk, and
                explainable AI outputs as
                separate visual modules
                before the final
                investment-support view.
              </p>
            </div>
        ...
        {!hasPreview && !loading && (
          <section className="surface-panel text-center">
            ...
            <p className="mx-auto mt-4 max-w-xl text-base leading-8 text-slate-600">
              ...
              dashboard and start the
              analysis before opening this
              module view.
            </p>
              ...
              <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr] lg:items-stretch">
                <div className="rounded-[28px] border border-white/10 bg-white/8 p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="eyebrow !text-blue-100">
                        Live insight board
                      </p>
                  ...
                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-2xl border border-white/10 bg-slate-950/20 p-4">
                      <p className="text-[10px] uppercase tracking-[0.2em] text-blue-100/60">
                        Ticker
                      </p>
              ...
              <div className="grid gap-3 md:grid-cols-4">
                {pipelineStatus.map(
                  (
                    ...
                    <div
                      className="pipeline-step border-white/10 bg-white/8 p-4"
                      key={label}
                    >
                      ...
                      <p className="mt-3 text-[10px] uppercase tracking-[0.2em] text-blue-100/50">
                        {label}
                      </p>
              ...
              <div className="relative overflow-hidden rounded-[34px] border border-white/12 bg-[linear-gradient(145deg,rgba(255,255,255,0.14),rgba(255,255,255,0.06))] p-6 shadow-[0_28px_80px_rgba(3,10,35,0.22)] md:p-8">
                ...
                <div className="relative mt-8 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
                  <div className="rounded-[30px] border border-white/10 bg-slate-950/25 p-5">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-blue-100/55">
                      Output flow
                    </p>

                    <div className="mt-5 space-y-3">
                      {componentOutputs.map(
                        (
                          output,
                          index
                        ) => {
                          const row = (
                            <div
                              className={`group relative rounded-[22px] border border-white/10 bg-white/7 p-4 transition ${
                                output.route
                                  ? "cursor-pointer hover:border-fuchsia-200/35 hover:bg-white/12"
                                  : ""
                              }`}
                            >
                              {index <
                                componentOutputs.length -
                                  1 && (
                                <div className="absolute left-[34px] top-[58px] hidden h-6 w-px bg-white/15 sm:block" />
                              )}
                  ...
                  <div className="grid gap-5">
                    {componentOutputs.map(
                      (output) => {
                        const card = (
                          <article
                            className={`relative overflow-hidden rounded-[30px] border border-white/10 bg-slate-950/25 p-6 transition ${
                              output.route
                                ? "cursor-pointer hover:border-fuchsia-300/35 hover:bg-slate-950/35"
                                : ""
                            }`}
                          >
                            <div
                              className={`absolute inset-y-0 left-0 w-1.5 bg-gradient-to-b ${output.accent}`}
                            />

                            <div
                              className={`absolute -right-16 -top-16 h-40 w-40 rounded-full bg-gradient-to-br ${output.accent} blur-3xl`}
                            />

                            <div className="relative flex flex-wrap items-start justify-between gap-4">
                              <div>
                                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-blue-100/55">
                                  Module{" "}
                                  {
                                    output.code
                                  }
              ...
              <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
                  {insightCards.map(
                    ([title, text]) => (
                      <div
                        className="interactive-card rounded-[24px] border border-white/10 bg-white/8 p-5"
                        key={title}
                      >
                ...
                <div className="rounded-[28px] border border-white/12 bg-white/10 p-6">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                      ...
                      <div className="mt-3 grid gap-3">
                        {factorChips.map(
                          (factor) => (
                            <div
                              className="rounded-2xl border border-white/10 bg-white/8 px-4 py-3 text-sm text-slate-200"
                              key={factor}
                            >
                              {factor}
                            </div>
                  ...
                  <Link
                    to={sentimentRoute}
                    className="mt-6 inline-flex rounded-full border border-fuchsia-300/25 bg-fuchsia-300/10 px-5 py-3 text-sm font-semibold text-fuchsia-100 transition hover:bg-fuchsia-300/20"
                  >
